...
server.get('/exportLanguage/:tenant/:course/download.zip', function (req, res, next) {
...
server.get('/exportLanguage/:tenant/:course', function (req, res, next) {
    ...
    if (currentUser && (currentUser.tenant._id == tenant)) {
      var outputplugin = app.outputmanager.getOutputPlugin(configuration.getConfig('outputPlugin'), function (error, plugin){
        if (error) {
          ...
          plugin.exportLanguage(course, req, res, function (error, result) {
            if (error) {
